...
// blocks its caller (typically a WebSocket read-loop goroutine).
const sourceBufferSize = 64

// Source turns a WebSocket read loop's per-message delivery into an
// essessey.Source. Wire Deliver as the callback the read loop invokes
// per inbound message; Next pulls events off in the order they arrived.
type Source struct {
	mu     sync.Mutex
